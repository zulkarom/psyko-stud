...
class Login2Model
{
     ...
     */
	 public static function loginadmin($user_name, $user_password)
    {
       if (empty($user_name)) {
	    ...
	    $result = self::validateAndGetUserAdmin($user_name);
        if (!$result) {
            return false;
        }
        ...
        // if user has checked the "remember me" checkbox, then write token 

        // successfully logged in, so we write all necessary data into the session and set "user_logged_in" to true
        self::setSuccessfulLoginIntoSession(
            $result->user_id, $result->user_name, $result->user_account_type
        );
    ...
    public static function login($matrik, $ic)
    {
        if (empty($user_name)) {
            Session::add('feedback_negative', 'Input Empty');
            return false;
	    ...
	    $result = self::validateAndGetUser($matrik, $ic);
        if (!$result) {
            return false;
        }
	 ...
	 * @return bool|mixed
	 */
	private static function validateAndGetUser($matrik, $ic)
	{
	...
	private static function validateAndGetUserAdmin($user_name)
	{
     ...
     */
    public static function setSuccessfulLoginIntoSession($user_id, $user_name , $user_account_type)
    {
        Session::init();
